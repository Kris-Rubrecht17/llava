...
pub type LLVMMemoryManagerFinalizeMemoryCallback =
    unsafe extern "C" fn(opaque: *mut c_void, err_msg: *mut *mut c_char) -> LLVMBool;
pub type LLVMMemoryManagerDestroyCallback = Option<unsafe extern "C" fn(opaque: *mut c_void)>;
...
unsafe extern "C" {
    ...
    pub fn LLVMCreateGenericValueOfPointer(P: *mut c_void) -> LLVMGenericValueRef;
    pub fn LLVMCreateGenericValueOfFloat(
        Ty: LLVMTypeRef,
        N: c_double,
    ) -> LLVMGenericValueRef;
    pub fn LLVMGenericValueIntWidth(GenValRef: LLVMGenericValueRef) -> c_uint;
    pub fn LLVMGenericValueToInt(
        GenVal: LLVMGenericValueRef,
        IsSigned: LLVMBool,
    ) -> c_ulonglong;
    pub fn LLVMGenericValueToPointer(GenVal: LLVMGenericValueRef) -> *mut c_void;
